...
mysql_select_db($database_conexion, $conexion);
$query_mos_user = sprintf("SELECT * FROM call_estado WHERE est_id = %s", GetSQLValueString($colname_mos_user, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  $updateSQL = sprintf("UPDATE call_estado SET est_nombre=%s, est_color=%s WHERE est_id=%s",
                       GetSQLValueString(strtoupper($est_nombre), "text"),
					   GetSQLValueString($_POST['est_color'], "text"),
					   GetSQLValueString($_POST['est_id'], "text"));
  mysql_select_db($database_conexion, $conexion);
  $Result1 = mysql_query($updateSQL, $conexion) or die(mysql_error());

$m = 'Registro Actualizado Correctamente';
  $updateGoTo = "estado.php?n=$m";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="box-header"> <h3 class="box-title"> 
       	    <i class="fa fa-retweet"></i> Editar Estado</h3></div>
                   
                  <div class="box-body">
                    <form method="post" name="form" action="<?php echo $editFormAction; ?>">
                     
                        <div class="form-group">Estado:
                          <input type="text" name="est_nombre" value="<?php echo htmlentities($row_mos_user['est_nombre'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" required>
                        </div>
                        
                        <div class="form-group">Color:
                          <select name="est_color" class="form-control">
...
do {  
?>
                            <option value="<?php echo $row_mos_user['est_color']?>"<?php if (!(strcmp($row_mos_user['est_color'], $row_mos_user['est_color']))) {echo "selected=\"selected\"";} ?>><?php echo $row_mos_user['est_color']?></option>
                            <?php
} while ($row_mos_user = mysql_fetch_assoc($mos_user));
  $rows = mysql_num_rows($mos_user);
  if($rows > 0) {
      mysql_data_seek($mos_user, 0);
	  $row_mos_user = mysql_fetch_assoc($mos_user);
  }
?>
                          </select>
