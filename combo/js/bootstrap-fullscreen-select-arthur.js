 ...
 * Author: Arthur Zapata
 * Website: www.craftpip.com
 * Contact: hey@craftpip.com
 *
 * Copyright 2015 bootstrap-select
...
(function ($) {
    ...
    Mobileselect.prototype = {
        ...
        _buildHTML: function () {
             ...
             */
            this.$c.find('.mobileSelect-title').html(this.title).end().find('.mobileSelect-todosbtn').html(this.buttontodos).end().find('.mobileSelect-savebtn').html(this.buttonSave).end().find('.mobileSelect-clearbtn').html(this.buttonClear).end().find('.mobileSelect-cancelbtn').html(this.buttonCancel).end();
        ...
        _updateBtnCount: function () {
            ...
            if (this.$triggerElement.is('button') && this.$triggerElement.hasClass('btn-mobileSelect-gen')) {
                var a = this.$triggerElement.find('.text'),
                        b = this.$e.val();
                if (b === null) {
                    a.html('Ninguno seleccionado');
                    return false;
                }
                if (this.isMultiple) {
                    if (b.length === 1) {
                        /*a.html(b);*/
						a.html('1 item seleccionado');
                    } else {
        ...
        _bindEvents: function () {
			...
			this.$c.find('.mobileSelect-todosbtn').on('click', function (e) {
                e.preventDefault();
				that.$listcontainer.find('.mobileSelect-control').addClass('selected');
                that.syncR();
				that.hide();
            });
        ...
        _unbindEvents: function () {
			...
			this.$c.find('.mobileSelect-todosbtn').unbind('click');
        },
         sync: function () {
...
//                    var label = $t.parent().is('optgroup') ? $t.parent().attr('label') : false;
    ...
    $.fn.mobileSelect.defaults = {
        template: '<div><div class="mobileSelect-title"></div><div class="list-container"></div><div class="mobileSelect-buttons"> <a href="#" class="mobileSelect-todosbtn"></a><a href="#" class="mobileSelect-savebtn"></a><a href="#" class="mobileSelect-clearbtn"></a><a href="#" class="mobileSelect-cancelbtn"></a></div></div>',
        title: 'Seleccionar',
		buttontodos: 'Todos',
        buttonSave: 'Guardar',
        buttonClear: 'Limpiar',
