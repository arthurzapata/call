...
mysql_select_db($database_conexion, $conexion);
$query_mos_user = sprintf("SELECT * FROM call_empresa WHERE emp_id = %s", GetSQLValueString($colname_mos_user, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE call_empresa SET emp_nombre=%s WHERE emp_id=%s",
...
<div class="box-header"> <h3 class="box-title"> 
       	    <i class="fa fa-user"></i> Editar Empresa</h3></div>
                   
                  <div class="box-body">
                    <form method="post" name="form" action="<?php echo $editFormAction; ?>">
                     
                        <div class="form-group">Empresa:
                          <input type="text" name="usu_nombre" value="<?php echo htmlentities($row_mos_user['emp_nombre'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" required>
                        </div>
                     </div>
                  <!-- body -->     
