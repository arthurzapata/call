...
elseif ($perid == 1) 
$query_mos_vertodos = "SELECT COUNT(*) AS cant FROM call_registro r inner join call_usuario u ON r.usu_id = u.usu_id where r.emp_id = ".$empid." and u.cor_id =".$usuid."";
else
...
$query_mos_estado = "SELECT  e.est_id,  r.emp_id,  COUNT(r.est_id) AS cant,  e.est_nombre, e.est_color FROM call_registro r RIGHT JOIN call_estado e ON r.est_id = e.est_id inner join call_usuario u ON r.usu_id = u.usu_id
    WHERE  r.emp_id = ".$empid." and u.cor_id =".$usuid." GROUP BY e.est_nombre";
elseif ($perid == 4)//admin) {
...
$totalPages_mos_registro = ceil($totalRows_mos_registro/$maxRows_mos_registro)-1;
	...
	if (!empty($_SERVER['QUERY_STRING'])) {
	  ...
	  foreach ($params as $param) {
		if (stristr($param, "pageNum_mos_registro") == false && 
			stristr($param, "totalRows_mos_registro") == false) {
		  array_push($newParams, $param);
		}
	  }
	  if (count($newParams) != 0) {
		$queryString_mos_registro = "&" . htmlentities(implode("&", $newParams));
	  }
	}
	$queryString_mos_registro = sprintf("&totalRows_mos_registro=%d%s", $totalRows_mos_registro, $queryString_mos_registro);
}
//busqueda
                    ...
                    <ul class="sidebar-menu">
                        ...
                        <div class="row" style="margin:10px 0px 10px 0px;">
                            <div class="col-md-12 text-center">
                            	<button class="btn btn-success btn-sm" onclick="window.location='excel.php?es=<?php echo $idestado;?>'">
      ...
      <td></td>
       ...
       <?php do { ?>
        <tr>
          ...
          <td><a href="detalle_reg.php?id=<?php echo $row_mos_registro['reg_id']; ?>"><?php echo $row_mos_registro['reg_id']; ?></a></td>
          ...
          <td><?php echo $row_mos_registro['reg_observaciones']; ?></td>
           <td><?php if ($perid != 2) { // Show if recordset empty ?>
            <div align="center"><a onclick="return confirm('¿Seguro que desea eliminar?')" href="delete_reg.php?id=<?php echo $row_mos_registro['reg_id']; ?>" title="Eliminar" class="hide-option"><button class="btn btn-primary btn-xs" type="button" data-toggle="tooltip" data-title="Eliminar"><i class="fa fa-trash-o"></i></button></a></div></td>
                 ...
                 <div class="row">
                  <div class="col-md-6">   
                      <div class="form-group">Teléfono:
                        ...
                        <input type="text" id="reg_telefono" name="reg_telefono" value="" class="form-control" maxlength="30">
                          </div>
                      </div>
                   </div>
                   <div class="col-md-6">   
                      <div class="form-group">Teléfono 2:
                        ...
                        <input type="text" id="reg_telefono2" name="reg_telefono2" value="" class="form-control" maxlength="30">
                          </div>
                      </div>
                   </div>
                 </div>
