...
$currentPage ='';
$queryString_mos_registro='';
$pageNum_mos_registro='';
...
elseif ($perid == 1)//coor
  $query_mos_vertodos = "SELECT COUNT(*) AS cant FROM call_registro r inner join call_usuario u ON r.usu_id = u.usu_id where r.emp_id = ".$empid." and u.cor_id =".$usuid." or u.usu_id=".$usuid."";
...
elseif ($perid == 1)
  $query_mos_estado = "SELECT  e.est_id,  r.emp_id,  COUNT(r.est_id) AS cant,  e.est_nombre, e.est_color FROM call_registro r RIGHT JOIN call_estado e ON r.est_id = e.est_id inner join call_usuario u ON r.usu_id = u.usu_id
    WHERE  r.emp_id = ".$empid." and u.cor_id =".$usuid." or u.usu_id=".$usuid." GROUP BY e.est_nombre";  
elseif ($perid == 4)//admin
...
mysql_select_db($database_conexion, $conexion);
if ($perid == 3)//super admin
$query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.pro_descripcion,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN producto c ON r.cur_id = c.pro_id left join call_usuario u on r.usu_id = u.usu_id inner join call_empresa em on r.emp_id = em.emp_id order by r.reg_id desc";	
elseif($perid == 1)
  $query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.pro_descripcion,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN producto c ON r.cur_id = c.pro_id inner join call_usuario u on r.usu_id = u.usu_id inner join call_empresa em on r.emp_id = em.emp_id where r.emp_id=".$empid." and u.cor_id = ". $usuid." or u.usu_id=".$usuid." order by reg_id desc";
elseif ($perid ==4 )//admin
$query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.pro_descripcion,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN producto c ON r.cur_id = c.pro_id inner join call_usuario u on r.usu_id = u.usu_id inner join call_empresa em on r.emp_id = em.emp_id where r.emp_id=".$empid." order by reg_id desc";	
else
$query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.pro_descripcion,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN producto c ON r.cur_id = c.pro_id inner join call_usuario u on r.usu_id = u.usu_id inner join call_empresa em on r.emp_id = em.emp_id where r.usu_id=".$usuid." order by reg_id desc";
...
mysql_select_db($database_conexion, $conexion);
if ($perid == 3)//super admin
$query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.pro_descripcion,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN producto c ON r.cur_id = c.pro_id inner join call_usuario u on r.usu_id = u.usu_id inner join call_empresa em on r.emp_id = em.emp_id WHERE reg_apellidos LIKE '%".$_POST['buscar']."%' order by reg_id desc";
...
elseif($perid == 1)
{
  $query_mos_empresa = "SELECT * FROM call_usuario where emp_id=".$empid." and cor_id=".$usuid." and per_id in (2) order by usu_nombre asc";
  $prefijo = 'usu';
}
  else
	{
	$query_mos_empresa = "SELECT * FROM call_usuario where emp_id=".$empid." and per_id in (2,1) order by usu_nombre asc";
	$prefijo = 'usu';
	}
...
$startRow_mos_registro = $pageNum_mos_registro * $maxRows_mos_registro;
	...
	$consulta1 = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.pro_descripcion,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN producto c ON r.cur_id = c.pro_id inner join call_usuario u on r.usu_id = u.usu_id inner join call_empresa em on r.emp_id = em.emp_id";
...
//Productos
mysql_select_db($database_conexion, $conexion);
$query_mos_Productos = "SELECT * FROM producto where pro_activo = 1 order by pro_descripcion asc";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  //mensje x curso
	...
	mysql_select_db($database_conexion, $conexion);
	$query_mos_msjexcurso = sprintf("SELECT * FROM producto WHERE pro_id = %s", GetSQLValueString($colname_mos_msjexcurso, "int"));
                ...
                <div class="box box-primary">
                  ...
                  <div class="box-body">
                  	<div class="row" style="margin-bottom:10px;">
                        ...
                        <?php if ($perid != 2) { // Show if recordset empty ?>
                        
               <div class="col-sm-6">
                <form id="formbus" name="formbus" action="" method="post">
				       <select  class="form-control mobileSelect" multiple name="curso[]">
    					 <?php 
do {  
?>
                          <option value="<?php echo $row_mos_Productos['pro_id']?>" <?php if (!(strcmp($row_mos_Productos['pro_id'], $row_mos_Productos['pro_id']))) {echo "SELECTED";} ?>><?php echo $row_mos_Productos['pro_descripcion']?></option>
                          <?php
} while ($row_mos_Productos = mysql_fetch_assoc($mos_Productos));
?>
    					</select>                         
                 <!--</div>
                 <div class="col-sm-3">-->
                        <select  class="form-control mobileSelect" multiple name="empresa[]">
    					 <?php 
do {  
?>
                          <option value="<?php echo $row_mos_empresa["".$prefijo."_id"]?>" <?php if (!(strcmp($row_mos_empresa["".$prefijo."_id"], $row_mos_empresa["".$prefijo."_id"]))) {echo "SELECTED";} ?>><?php echo $row_mos_empresa["".$prefijo."_nombre"]?></option>
                          <?php
               ...
               <?php } // Show if recordset empty ?>         
                		<div class="col-sm-4 search-form">
                            <form name="formb" id="formb" action="" method="post" class="text-right">
                                  <div class="input-group">                                          
                                     <input type="text" name="buscar" class="form-control" placeholder="Buscar Postulante ...">
 ...
 <form name="frm" action="" method="post">
  <table class="table table-bordered table-striped table-hover table-condensed tablesorter">
      ...
      <?php do { ?>
		...
		<?php  echo $row_mos_registro['reg_id']; ?>
          </a>
          </td>
