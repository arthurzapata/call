...
mysql_select_db($database_conexion, $conexion);
$query_mos_curso = sprintf("SELECT * FROM call_curso WHERE cur_id = %s", GetSQLValueString($colname_mos_curso, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  //
  $sContent=stripslashes($_POST['cur_descripcion']);//remove slashes (/)
    $sContent=ereg_replace("'","''",$sContent);//fix SQL
  //
  $updateSQL = sprintf("UPDATE call_curso SET cur_nombre=%s, cur_descripcion=%s, cur_activo=%s, cur_remitente =%s, cur_asunto=%s WHERE cur_id=%s",
                       GetSQLValueString($_POST['cur_nombre'], "text"),
                       GetSQLValueString($sContent, "text"),
                       GetSQLValueString(isset($_POST['cur_activo']) ? "true" : "", "defined","1","0"),
                       GetSQLValueString($_POST['cur_remitente'], "text"),
                       GetSQLValueString($_POST['cur_asunto'], "text"),
...
<div class="row">
                ...
                <div class="box box-primary">
                  ...
                  <h3 class="box-title"><i class="fa fa-briefcase"></i> Editar Curso</h3>
                  </div>
...
<div class="box-body">
                    <form method="post" name="form1" action="<?php echo $editFormAction; ?>">
                        ...
                        <div class="form-group">Curso:
                          <input type="text" name="cur_nombre" value="<?php echo htmlentities($row_mos_curso['cur_nombre'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" required>
                        </div>
                        <div class="form-group">Remitente:
                          <input type="text" name="cur_remitente" value="<?php echo htmlentities($row_mos_curso['cur_remitente'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" maxlength="100">
                        </div>
                        <div class="form-group">Asunto:
                          <input type="text" name="cur_asunto" value="<?php echo htmlentities($row_mos_curso['cur_asunto'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" maxlength="100">
                        </div>
                         <div class="form-group">Descripción:
                          <textarea id="cur_descripcion" name="cur_descripcion"  class="form-control">
                             <?php
  ...
  echo $row_mos_curso['cur_descripcion'];
  if(isset($_POST['cur_descripcion']))
    {
    $sContent=stripslashes($_POST['cur_descripcion']); /*** remove (/) slashes ***/
  ...
  oEdit1.cmdAssetManager = "modalDialogShow('assetmanager/assetmanager.php',640,465)"; //Command to open the Asset Manager add-on.  
    oEdit1.REPLACE("cur_descripcion");//Specify the id of the textarea here
  </script>
                        </div>
                        <div class="form-group">Activo:
                          <input type="checkbox" name="cur_activo" value=""  <?php if (!(strcmp(htmlentities($row_mos_curso['cur_activo'], ENT_COMPAT, 'UTF-8'),1))) {echo "checked=\"checked\"";} ?>>
                        </div>
