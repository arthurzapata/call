...
mysql_select_db($database_conexion, $conexion);
  if ($perid==4) 
    $query_mos_subcat = "SELECT usu_id,usu_nombre,per_id FROM call_usuario where emp_id = ".$empid." and per_id in (1) and usu_activo = 1 order by usu_nombre";
  elseif ($perid==3) {
    $query_mos_subcat = "SELECT usu_id,usu_nombre,per_id FROM call_usuario where emp_id = ".$empid." and per_id in (1,2) and usu_activo = 1 order by usu_nombre";
  }
  else
  {  // tiempo completo
    $query_mos_subcat = "SELECT usu_id,usu_nombre,per_id FROM call_usuario where emp_id = ".$empid." and per_id in (2) and usu_activo = 1 and cor_id = ". $usuid." and tc = 1 order by usu_nombre";
  $mos_subcat = mysql_query($query_mos_subcat, $conexion) or die(mysql_error());
  $row_mos_subcat = mysql_fetch_assoc($mos_subcat);
  /// medio tiempo
      $query_mos_subcatpartime = "SELECT usu_id,usu_nombre,per_id FROM call_usuario where emp_id = ".$empid." and per_id in (2) and usu_activo = 1 and cor_id = ". $usuid." and tc = 0 order by usu_nombre";
  $mos_subcatpartime = mysql_query($query_mos_subcatpartime, $conexion) or die(mysql_error());
  $row_mos_subcatpartime = mysql_fetch_assoc($mos_subcatpartime);
  /////
  } 
////////////
//registros
$maxRows_mos_registro = 1000;
...
$query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.cur_nombre,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN call_curso c ON r.cur_id = c.cur_id inner join call_usuario u on r.usu_id = u.usu_id and u.per_id in (1,4) inner join call_empresa em on r.emp_id = em.emp_id 
  where u.usu_id=".$usuid." and r.emp_id=".$empid." and  r.est_id=1 order by reg_id desc";
elseif ($perid == 4)  //admin
    $query_mos_registro = "SELECT reg_id,reg_codigo,DATE_FORMAT(reg_fecha,'%e/%m/%Y')as reg_fecha,r.est_id,r.cur_id,DATE_FORMAT(reg_fechareg,'%e/%m/%Y') as reg_fechareg, r.usu_id,  CONCAT(IFNULL(reg_apellidos,''),' ',IFNULL(reg_nombres,'')) AS cliente,reg_formacion, reg_observaciones,reg_ciudad,reg_pais,reg_email,reg_telefono,e.est_color,c.cur_nombre,u.usu_nombre,em.emp_nombre FROM call_registro r INNER JOIN call_estado e ON r.est_id = e.est_id INNER JOIN call_curso c ON r.cur_id = c.cur_id left join call_usuario u on r.usu_id = u.usu_id and u.per_id in (1,4) inner join call_empresa em on r.emp_id = em.emp_id 
  where  r.emp_id=".$empid." and r.est_id=1 and r.reg_derivado  = 0 order by reg_id desc";
...
{
		//update
		$query2="update call_registro set emp_id=". $emp .",reg_derivado = 1, usu_id = ".$user." WHERE reg_id = '".$checkbox2[$i]."'";
...
{
$checkbox2 = isset($_POST['casilla']) ? $_POST['casilla'] : NULL;
$checkboxuser = isset($_POST['casuser']) ? $_POST['casuser'] : NULL;
$checkboxuserpt = isset($_POST['casuserpt']) ? $_POST['casuserpt'] : NULL;

//$user = $_POST['usu_id'];
//$emp = $_POST['emp_id'];
$totalpt =  sizeof($checkboxuserpt); // total registro pt
$totalreg = sizeof($checkbox2); // total registros
$totalcall = sizeof($checkboxuser); // total call
$resul = $totalreg / $totalcall;
$partime = $result / 2 ; 
$i = 0;
$j=0;
...
for ($k=0; $k < $totalpt; $k++) 
{ 
    for ($pt=0; $pt < $partime; $pt++) 
    { 
       $query2="update call_registro set emp_id=". $empid .",reg_derivado = 1, usu_id = ".$checkboxuserpt[$k]." WHERE reg_id = '".$checkbox2[$i]."'";
    $Result1 = mysql_query($query2, $conexion) or die(mysql_error());
      $i++;
    }
}
...
// time completo
for ($i=$i;$i<sizeof($checkbox2);$i++)
{
    if ($j == $cant){
      $j=0;
    }
    //update
    $query2="update call_registro set emp_id=". $empid .",reg_derivado = 1, usu_id = ".$checkboxuser[$j]." WHERE reg_id = '".$checkbox2[$i]."'";
    $Result1 = mysql_query($query2, $conexion) or die(mysql_error());
...
for ($i=0;$i< $totalreg ;$i++) // total de registros
{
    for ($j=0;$j < $totalcall ;$j++) // call center o coord
    {
 
        if ($usuid = 1) // si es part time
            if ($k == $partime){
              $k=0;
            }
            $query2="update call_registro set emp_id=". $empid .",reg_derivado = 1, usu_id = ".$checkboxuser[$j]." WHERE reg_id = '".$checkbox2[$i]."'";
...
<button class="btn btn-success btn-sm" onclick="window.location='excel.php'" disabled="true" >
                            	<i class="fa fa-cloud-download"></i> Descargar EXCEL</button>
                            </div>
                        </div>
                        <div class="row" style="margin:0px 0px 10px 0px;">   
                            <div class="col-md-12 text-center">
                            	<button class="btn btn-danger btn-sm" onclick="window.location='pdf.php'" disabled="true">
...
<div class="row" >
    <div class="clas-md-6" >
     <table class="table table-bordered table-striped table-hover table-condensed tablesorter">
      <tr>
        <td><strong></strong></td>
        <td align="center"><strong>Usuario TC</strong></td> 
      </tr>
      <?php do { ?>
        <tr>
          <td> 
           <input name="casuser[]" type="checkbox" id="casuser[]" value="<?php echo $row_mos_subcat['usu_id']; ?>">
              
          </td>
            <td><?php echo $row_mos_subcat['usu_nombre']; ?></td>
          </tr>
           <?php } while ($row_mos_subcat = mysql_fetch_assoc($mos_subcat)); ?>
           </table>
    </div>
...
<div class="clas-md-6" >
     <table class="table table-bordered table-striped table-hover table-condensed tablesorter">
      <tr>
        <td><strong></strong></td>
        <td align="center"><strong>Usuario PT</strong></td> 
      </tr>
      <?php do { ?>
        <tr>
          <td> 
           <input name="casuserp[]" type="checkbox" id="casuserpt[]" value="<?php echo $row_mos_subcatpartime['usu_id']; ?>">
              
          </td>
            <td><?php echo $row_mos_subcatpartime['usu_nombre']; ?></td>
          </tr>
           <?php } while ($row_mos_subcatpartime = mysql_fetch_assoc($mos_subcatpartime)); ?>
           </table>
    </div>
</div>
<br>
