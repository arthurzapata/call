...
$query_mos_perfil = "SELECT * FROM call_perfil where per_id in (2,1)";
...
$colname_mos_user = "-1";
if (isset($_GET['id'])) {
  $colname_mos_user = $_GET['id'];
}
mysql_select_db($database_conexion, $conexion);
$query_mos_user = sprintf("SELECT * FROM call_usuario WHERE usu_id = %s", GetSQLValueString($colname_mos_user, "int"));
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
 $updateSQL = sprintf("UPDATE call_usuario SET usu_nombre=%s, per_id=%s,emp_id=%s,usu_clave=%s, usu_activo=%s WHERE usu_id=%s",
...
<div class="box-header"> <h3 class="box-title"> 
       	    <i class="fa fa-user"></i> Editar Usuario</h3></div>
                   
                  <div class="box-body">
                    <form method="post" name="form" action="<?php echo $editFormAction; ?>">
                     
                        <div class="form-group">Usuario:
                          <input type="text" name="usu_nombre" value="<?php echo htmlentities($row_mos_user['usu_nombre'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" required>
                        </div>
                        <div class="form-group">Contraseña:
                          <input type="password" name="usu_clave" value="<?php echo htmlentities($row_mos_user['usu_clave'], ENT_COMPAT, 'UTF-8'); ?>" class="form-control" required>
                        </div>
...
do {  
?>
                            <option value="<?php echo $row_mos_perfil['per_id']?>" <?php if (!(strcmp($row_mos_perfil['per_id'], htmlentities($row_mos_user['per_id'], ENT_COMPAT, 'UTF-8')))) {echo "SELECTED";} ?>><?php echo $row_mos_perfil['per_nombre']?></option>
                          <?php
...
do {  
?>
                            <option value="<?php echo $row_mos_empresa['emp_id']?>" <?php if (!(strcmp($row_mos_empresa['emp_id'], htmlentities($row_mos_empresa['emp_id'], ENT_COMPAT, 'UTF-8')))) {echo "SELECTED";} ?>><?php echo $row_mos_empresa['emp_nombre']?></option>
                          <?php
...
?>
                        ...
                        <div class="form-group">Activo:
                          ...
                          <input type="checkbox" name="usu_activo" value=""  <?php if (!(strcmp(htmlentities($row_mos_user['usu_activo'], ENT_COMPAT, 'UTF-8'),1))) {echo "checked=\"checked\"";} ?>>
                        </div>
                     </div>
                  <!-- body -->     
              ...
              <a href="usuario.php" style="margin-left:10px;">
                 <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-times"></i> Cancelar</button></a>
                    ...
                    <input type="hidden" name="usu_id" value="<?php echo $row_mos_user['usu_id']; ?>">
